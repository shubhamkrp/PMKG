...
import pandas as pd
symp_file="Neurology.csv"
symp_df=pd.read_csv(symp_file, delimiter=';', on_bad_lines='skip')
symp_term=symp_df["Name"]
symp_term=symp_term.loc[35:50]
print(symp_term.head())

for term in symp_term:
    term=term.lower()
    search_term = f'"{term}" AND (1970/01/01[PDAT] : 2023/12/31[PDAT])'
    ids = search_pubmed(search_term, max_results=999999)
    if len(ids)==0:
        with open('diseases_with_no_articles.txt', 'a') as f:
            f.write(f"{term}\n")
        continue
